...
@Controller
public class SaatlikController {
	...
	public String home(Locale locale, Model model, @RequestParam(value = "il", required = false) String il) {
		...
		ResponseEntity<IlModel[]> result = restTemplate.getForEntity(
					"http://212.175.180.28/api/merkezler?il=" + il, 
					IlModel[].class);
		IlModel ilBilgisi = result.getBody()[0];
		
		ResponseEntity<SaatlikHavaDurumuModel[]> result2 = restTemplate.getForEntity(
					"http://212.175.180.28/api/tahminler/saatlik?istno=" + ilBilgisi.getSaatlikTahminIstNo(), 
					SaatlikHavaDurumuModel[].class);
		SaatlikHavaDurumuModel saatlikHavaBilgisi = result2.getBody()[0];
